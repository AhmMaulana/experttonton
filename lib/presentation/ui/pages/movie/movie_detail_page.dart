  ...
  void initState() {
    ...
    Future.microtask(() {
      context
          ...
          .add(GetDetailMovieEvent(
          widget.id)
      );

      context
          .read<RecommendationMovieBloc>()
          .add(GetRecommendationMovieEvent(
          widget.id)
      );

      context
          .read<WatchlistMovieBloc>()
          .add(GetStatusMovieEvent
        (widget.id)
      );
      }
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: BlocListener<WatchlistMovieBloc, WatchlistMovieState>(
        ...
        child: BlocBuilder<DetailMovieBloc,
            DetailMovieState>(
          builder: (context, state) {
            ...
            state is DetailMovieLoaded)
            {
              final movie = state.movieDetail;
              bool isAddedToWatchlist = (context.watch<WatchlistMovieBloc>()
                  .state
              is WatchlistMovieStatusLoaded)
                  ? (context.read<WatchlistMovieBloc>().state

              as WatchlistMovieStatusLoaded)
                  .result
                  : false;
                          ...
                          children: [
                            ...
                            BlocBuilder<RecommendationMovieBloc,
                              ...
                              builder: (context, state) {
                                ...
                                state is RecommendationMovieError)
                                {
                                  return Text(state.message);

                                }
                                else if (
                                state is RecommendationMovieLoaded)
                                {
